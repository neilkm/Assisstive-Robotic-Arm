...
BluetoothUartDriver::BluetoothUartDriver()
    : fd_(-1)
{
}
...
bool BluetoothUartDriver::openPort(const std::string& device, int baudrate, std::string* error)
{
    closePort();
...
void BluetoothUartDriver::closePort()
{
    if (fd_ >= 0) {
        ::close(fd_);
        fd_ = -1;
...
bool BluetoothUartDriver::isOpen() const
{
    return fd_ >= 0;
}

ssize_t BluetoothUartDriver::readBytes(uint8_t* data, size_t len, int timeout_ms, std::string* error)
{
    if (fd_ < 0) {
        setError(error, "read requested before Bluetooth UART port was opened");
        return -1;
    }
...
bool BluetoothUartDriver::writeAll(const uint8_t* data, size_t len, std::string* error)
{
    if (fd_ < 0) {
        setError(error, "write requested before Bluetooth UART port was opened");
        return false;
    }
    if (data == nullptr && len != 0u) {
        ...
        if (count < 0) {
            if (errno == EAGAIN || errno == EWOULDBLOCK || errno == EINTR) {
                ...
                if (poll(&pfd, 1, 1000) <= 0) {
                    setError(error, "timed out waiting for Bluetooth UART transmit space");
                    return false;
                }
                continue;
            }
